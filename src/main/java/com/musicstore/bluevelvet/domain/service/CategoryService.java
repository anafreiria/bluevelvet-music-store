...
public class CategoryService {
    ...
    public String generateHierarchyString(CategoryResponse root, int level) {
        StringBuilder sb = new StringBuilder();

        // 1. Criar a indentação (recuo) baseada no nível atual
        // Se o nível for 0, não mete traços. Se for 1, mete "--", etc.
        for (int k = 0; k < level; k++) {
            sb.append("--");
        }

        // 2. Adicionar o nome da categoria atual e pular uma linha
        sb.append(root.getName()).append("\n");

        // 3. Verificar se há filhos e chamar o método para eles (Recursividade)
        // Importante verificar se a lista não é nula para evitar erros
        if (root.getChildren() != null) {
            for (CategoryResponse child : root.getChildren()) {
                // Chamamos o mesmo método, mas aumentamos o nível (level + 1)
                sb.append(generateHierarchyString(child, level + 1));
            }
        }
